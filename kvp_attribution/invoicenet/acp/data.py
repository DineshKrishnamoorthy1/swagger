...
class InvoiceData(Data):
    ...
    def shapes(self, provided_fields):
        return (
            (None, 5),  # i
            (None,),  # v
            (None,),  # s
            ...
            (InvoiceData.seq_out[provided_fields[self.field]],)  # target
        )
    ...
    def _load_document(self, doc_id, provided_fields):
        ...
        if provided_fields[self.field] == FIELD_TYPES["amount"]:
            target = self._preprocess_amount(target)
        target = InvoiceData.encode_sequence(target, self.seq_out[provided_fields[self.field]])
    ...
    def sample_generator(self, provided_fields):
        exceptions = 0
        np.random.seed(0)
        ...
        for i, doc_id in enumerate(self.filenames):
            try:
                yield self._load_document(doc_id.strip(), provided_fields)
    ...
    @staticmethod
    def create_dataset(data_dir, field, batch_size, provided_fields):
        data = InvoiceData(field=field, data_dir=data_dir)
        shapes, types = data.shapes(provided_fields), data.types()

        def _transform(i, v, s, *args):
            return (tf.SparseTensor(i, v, s),) + args

        generator = functools.partial(data.sample_generator, provided_fields=provided_fields)
